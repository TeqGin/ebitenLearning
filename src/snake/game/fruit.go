...
func LoadFruit(path string, scalar float64, cfg *utils.Config) *Fruit {
	...
	return &Fruit{
		...
		Y:      rand.Intn(cfg.Height / gridSize),
	}
}

func (f *Fruit) Generate(cfg *utils.Config) {
	f.X = rand.Intn(cfg.Width / gridSize)
	f.Y = rand.Intn(cfg.Height / gridSize)
}
